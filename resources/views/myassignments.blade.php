<x-app-layout>
    ...
    <h2 class="font-semibold text-2xl text-white leading-tight mb-6">
        ...
        <a class="mb-4  text-white course-link hover:font-bold" href="{{ route('course.mycourse', ['course' => $course->id]) }}"> {{__('messages.Back to Course')}}</a>
        @if ($assignments->count() > 0)
            ...
            @endforeach
        @else
           <p> {{__('messages.No assignments at') }}</p>
        @endif
        <button class="inline-block bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
        <a href="{{ route('assignments.create', ['course' => $course]) }}" class="text-grey-800">{{__('messages.Add a new assignment') }}</a>
        </button>
    </div>
</div>
</x-app-layout>
